/*
 * 文件名: anti_aliasing_effect.cpp
 * 功能: 基于SDF的抗锯齿特效 - 可分离两 pass 实现
 *       Pass1: 横向高斯模糊
 *       Pass2: 纵向高斯模糊 + SDF Alpha 遮罩
 */

#include "anti_aliasing_effect.h"
...
static std::wstring GetAADLLDirectory() {
    wchar_t path[MAX_PATH];
    HMODULE hModule = NULL;
    GetModuleHandleExW(
        GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
        (LPCWSTR)&GetAADLLDirectory,
        &hModule
    );
...
bool AntiAliasingEffect::Initialize(ID3D11Device* device) {
    ...
    HRESULT hr = device->CreateBuffer(&cbDesc, nullptr, &constantBuffer_);
    if (FAILED(hr)) {
        return false;
    }
...
bool AntiAliasingEffect::LoadShaders(ID3D11Device* device) {
    HRESULT hr;
    std::wstring dllDir = GetAADLLDirectory();

    // 顶点 Shader（全屏三角形，内联编译）
    const char* vsCode = R"(
    ...
    hr = D3DCompile(vsCode, strlen(vsCode), nullptr, nullptr, nullptr,
                    "VSMain", "vs_5_0", 0, 0, &vsBlob, &vsErr);
    if (FAILED(hr)) {
    ...
    // Pass1 横向 PS
    hr = LoadPSFromCSO(device, dllDir + L"anti_aliasing_effect_horz_ps.cso", &pixelShaderHorz_);
    if (FAILED(hr)) { return false; }

    // Pass2 纵向+SDF PS
    hr = LoadPSFromCSO(device, dllDir + L"anti_aliasing_effect_vert_ps.cso", &pixelShaderVert_);
    if (FAILED(hr)) { return false; }
...
bool AntiAliasingEffect::CreateSamplerState(ID3D11Device* device) {
...
bool AntiAliasingEffect::EnsureIntermediateTexture(UINT width, UINT height) {
...
bool AntiAliasingEffect::Apply(
    ...
    // ============================================================
    // Pass 2: 纵向模糊 + SDF Alpha 遮罩 → outputRTV
    //   t0 = 横向模糊结果, t1 = SDF, t2 = 原始输入
    // ============================================================
...
void AntiAliasingEffect::Shutdown() {
    if (intermediateRTV_)     { intermediateRTV_->Release();     intermediateRTV_     = nullptr; }
    if (intermediateSRV_)     { intermediateSRV_->Release();     intermediateSRV_     = nullptr; }
    if (intermediateTexture_) { intermediateTexture_->Release(); intermediateTexture_ = nullptr; }
...
size_t AntiAliasingEffect::ParamsSize() const { return sizeof(AntiAliasingEffectParams); }

void AntiAliasingEffect::BuildParams(const ParamMap& params, float texW, float texH,
    float posX, float posY, void* outBuf, size_t bufSize) const {
    if (bufSize < sizeof(AntiAliasingEffectParams)) return;
    auto fget = [&](const char* k, float def) -> float {
        auto it = params.find(k); return it != params.end() ? it->second : def;
    };
    AntiAliasingEffectParams p;
    p.posX = posX; p.posY = posY; p.scaleX = 1.0f; p.scaleY = 1.0f;
    p.texWidth = texW; p.texHeight = texH;
    p.blurRadius = fget("blur_radius", p.blurRadius);
    p.edgeRange  = fget("edge_range",  p.edgeRange);
    p.strength   = fget("strength",    p.strength);
    memcpy(outBuf, &p, sizeof(AntiAliasingEffectParams));
}

REGISTER_EFFECT(GPUEffectType::AntiAliasing, AntiAliasingEffect, 80)