/*
 * 文件名: chromatic_aberration_effect.h
 * 功能: 色散特效 - 基于SDF的RGB通道分离效果
 */
...
class ChromaticAberrationEffect : public GPUEffectBase {
public:
    ChromaticAberrationEffect() = default;
    ~ChromaticAberrationEffect() { Shutdown(); }
    ...
    GPUEffectType GetType() const override { 
        return GPUEffectType::ChromaticAberration; 
    }
