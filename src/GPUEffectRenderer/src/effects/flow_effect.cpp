...
bool FlowEffect::Initialize(ID3D11Device* device) {
    if (!device) {
        return false;
    }
...
bool FlowEffect::LoadShaders(ID3D11Device* device) {
    HRESULT hr;
    ...
    // 尝试从.cso加载
    std::wstring csoPath = dllDir + L"flow_effect_ps.cso";
    ...
    if (psFile.is_open()) {
        ...
        std::vector<std::wstring> hlslPaths = {
            dllDir + L"..\\src\\effects\\flow_effect.hlsl",
            dllDir + L"flow_effect.hlsl",
            L"src\\effects\\flow_effect.hlsl",
            L"GPUEffectRenderer\\src\\effects\\flow_effect.hlsl"
        };
...
bool FlowEffect::CreateConstantBuffer(ID3D11Device* device) {
    D3D11_BUFFER_DESC cbDesc = {};
    cbDesc.ByteWidth = sizeof(FlowEffectParams);
...
bool FlowEffect::CreateSamplerState(ID3D11Device* device) {
...
bool FlowEffect::Apply(
    ...
    if (SUCCEEDED(hr)) {
        memcpy(mapped.pData, params, sizeof(FlowEffectParams));
...
// ============================================================
//  泛型参数构建（供 GPUEffectRenderer 调用）
// ============================================================

#include "../gpu_effect_registry.h"

size_t FlowEffect::ParamsSize() const { return sizeof(FlowEffectParams); }

void FlowEffect::BuildParams(const ParamMap& params, float texW, float texH,
                              float posX, float posY, void* outBuf, size_t bufSize) const {
    if (bufSize < sizeof(FlowEffectParams)) return;
    ...
    p.flowStrength = fget("flow_strength", p.flowStrength);
    p.flowWidth    = fget("flow_width",    p.flowWidth);
    p.flowFalloff  = fget("flow_falloff",  p.flowFalloff);
    p.texWidth = texW; p.texHeight = texH;
    memcpy(outBuf, &p, sizeof(FlowEffectParams));
}
